...
/**
 * This class may be used with TreeNodeContentProvider to create a Tree of
 * editable items.
 * 
 * The classes LabelNode, NumericNode and ObjectNode are generic and may be used
 * elsewhere. They have not been moved somewhere generic yet because they create
 * a dependency on jscience.
 * 
 * @author Matthew Gerring
 *
 */
@SuppressWarnings({ "rawtypes", "unchecked" })
public class GridNumericNode<E extends Quantity> extends LabelNode {
	...
	/**
	 * allowedUnits does not have to be E intentionally For instance angstrom
	 * and eV are compatible using jscience which knows about the Planck
	 ...
	 * @param unit
	 */
	public GridNumericNode(String label, LabelNode parent, Unit<E> unit) {
		this(label, parent, unit, "#0.####");
	}

	public GridNumericNode(String label, LabelNode parent, Unit<E> unit, String numberFormat) {
		super(label, parent);
	...
	public void addAmountListener(AmountListener<E> l) {
		if (listeners == null)
			listeners = new HashSet<AmountListener<E>>(3);
		listeners.add(l);
	}
	...
	public void addUnitListener(UnitListener l) {
		if (unitListeners == null)
			unitListeners = new HashSet<UnitListener>(3);
		unitListeners.add(l);
	}
	...
	public void setValue(Amount<E> val, AmountListener<E>... ignored) {
		...
		if (value != null) {
			...
			Unit oldUnit = oldValue != null ? oldValue.getUnit()
					: (defaultValue != null ? defaultValue.getUnit() : defaultUnit);
	...
	public boolean mergeValue(TreeNode node) throws Throwable {
		...
		if (node instanceof GridNumericNode) {
			GridNumericNode nn = (GridNumericNode) node;
	...
	private Amount parseValue(Object val) throws Throwable {
		try {
			...
			try {
				return (Amount) Amount.valueOf(val.toString()); // e.g. "100.0
																// mm"
	...
	/**
	 * Sets the default value and sets the bounds to the values 0 and 10000.
	 * 
	 * @param value
	 */
	...
	private boolean isInAngstroms(Amount val, Unit to) {
		boolean isAngstrom = allowedUnits != null && allowedUnits.contains(NonSI.ANGSTROM)
				&& allowedUnits.contains(NonSI.ELECTRON_VOLT);
		if (!isAngstrom)
			return false;
	...
	public void setUnit(Unit<E> unit) {
		if (value != null)
			value = Amount.valueOf(value.doubleValue(unit), unit);
		if (defaultValue != null)
			defaultValue = Amount.valueOf(defaultValue.doubleValue(unit), unit);
		fireUnitChanged(unit);
	}
	...
	public void setUnits(Unit... allowedUnits) {
		...
		if (defaultValue != null)
			defaultValue = convertToNewSet(defaultValue, allowedUnits);
	}
	...
	public String[] getUnitsString() {
		...
		for (int i = 0; i < ret.length; i++)
			ret[i] = allowedUnits.get(i).toString();
		return ret;
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((allowedUnits == null) ? 0 : allowedUnits.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		GridNumericNode other = (GridNumericNode) obj;
		if (allowedUnits == null) {
