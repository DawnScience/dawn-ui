...
public class FitGaussianConvolutedFermiActor extends AbstractDataMessageTransformer {
	...
	public FitGaussianConvolutedFermiActor(CompositeEntity container, String name)
		...
		fitConvolution.addChoice("True");
		fitConvolution.addChoice("False");
		registerConfigurableParameter(fitConvolution);
	}

	private AbstractDataset getFermiDS(AbstractDataset xAxis, double mu, double kT, double scaleM, double scaleC, double constant) {
		...
		fermiDS.imultiply(sl.makeDataset(xAxis));
		fermiDS.iadd(constant);
		return fermiDS;
	}

	private AFunction FitGaussianConvFermi(final AbstractDataset xAxis, final AbstractDataset values, final AFunction fitFunction) {

		if (!(fitFunction instanceof FermiGauss)) {
			throw new IllegalArgumentException("Input function must be of type FermiGauss");
		}
		...
		MultivariateFunction f = new MultivariateFunction() {
			...
			public double value(double[] p) {
				...
				double p0 = deNormalizeParameter(p[0], fitFunction.getParameter(0));
				double p1 = deNormalizeParameter(p[1], fitFunction.getParameter(2));
				double p2 = deNormalizeParameter(p[2], fitFunction.getParameter(3));
				double p3 = deNormalizeParameter(p[3], fitFunction.getParameter(4));
				...
				//				try {
					//					SDAPlotter.plot("Plot 1", xAxis, new IDataset[] {fermiDS, values});
				//				} catch (Exception e) {
		...
		double[] start = new double[] {normalizeParameter(fitFunction.getParameterValue(0), fitFunction.getParameter(0)),
		...
		SimplexOptimizer simOpt = new SimplexOptimizer(0.001,0.001);
		simOpt.setSimplex(new NelderMeadSimplex(4));
		PointValuePair result = simOpt.optimize(1000, f, GoalType.MINIMIZE, start);
		...
		if (fitConvolution.getExpression().contains("True")) {
			...
			MultivariateFunction f1 = new MultivariateFunction() {
				...
				public double value(double[] p) {
					...
					double p2 = deNormalizeParameter(p[2], fitFunction.getParameter(3));
					double p3 = deNormalizeParameter(p[3], fitFunction.getParameter(4));
					double p4 = deNormalizeParameter(p[4], fitFunction.getParameter(5));
			...
			// preform the optimisation
			start = new double[] {r[0],r[1],r[2],r[3],
	...
	/**
	 * Takes in a real value and returns a value between 0 and 1;
	 * this value can be reverted using deNormalizeParameter
	 * @param value
	 * @param iParameter
	 * @return
	 */
	 ...
	 * @return
	 */
	public static double deNormalizeParameter(double value, IParameter iParameter) {
		double max = iParameter.getUpperLimit();
		double min = iParameter.getLowerLimit();
		double range = max-min;
