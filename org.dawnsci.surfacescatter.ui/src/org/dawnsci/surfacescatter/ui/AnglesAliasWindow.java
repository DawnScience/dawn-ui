...
public class AnglesAliasWindow extends Composite {
	...
	private boolean useInternalFluxFlag = false;

	public AnglesAliasWindow(Composite parent, int style, SurfaceScatterPresenter ssp, SurfaceScatterViewStart ssvs) {
	...
	public void createContents() {
		...
		Group geometricParametersReflecOutline = new Group(folder, SWT.NULL);
		GridLayout geometricParametersReflecOutlineLayout = new GridLayout(1, true);
		GridData geometricParametersReflecOutlineData = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
		geometricParametersReflecOutline.setLayout(geometricParametersReflecOutlineLayout);
		geometricParametersReflecOutline.setLayoutData(geometricParametersReflecOutlineData);

		Button useInternalFlux = new Button(geometricParametersReflecOutline, SWT.PUSH | SWT.FILL);
		useInternalFlux.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		useInternalFlux.setText("Use Internal Flux");

		Group geometricParametersReflecTiles = new Group(geometricParametersReflecOutline, SWT.NULL);
		...
		reflectivityFluxMap = new EnumMap<ReflectivityFluxParametersAliasEnum, Combo>(
		...
		useInternalFlux.addSelectionListener(new SelectionListener() {

			@Override
			public void widgetSelected(SelectionEvent e) {

				fluxPathSelection.setEnabled(useInternalFluxFlag);
				fluxPath.setEnabled(useInternalFluxFlag);

				useInternalFluxFlag = !useInternalFluxFlag;

				if (useInternalFluxFlag) {
					useInternalFlux.setText("Select External Flux Calibration File");
					updateFluxWithInternalOptions(true);
				} else {
					useInternalFlux.setText("Use Internal Flux");
					pokeFluxFileText();
				}
		...
		paramsReflec.setControl(geometricParametersReflecOutline);
	...
	public void updateAllWithOptions(String[] options, boolean useDefault) {
	...
	private void updateFluxWithInternalOptions(boolean useDefault) {

		String[] options = angleCombosReflectivity[0].getItems();

		updateFluxWithOptions(options, useDefault);

	}

	private void updateFluxWithOptions(String[] options, boolean useDefault) {
	...
	public void anglesUpdate(SelectionEvent e, SXRDAngleAliasEnum angle) {
	...
	public void anglesUpdate(SelectionEvent e, ReflectivityAngleAliasEnum angle) {
	...
	public void fluxUpdate(SelectionEvent e, ReflectivityFluxParametersAliasEnum angle) {
	...
	public boolean isUseInternalFluxFlag() {
		return useInternalFluxFlag;
	}

	private void pokeFluxFileText() {
