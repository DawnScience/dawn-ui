...
public class AttenuationCorrectedOutput {
	...
	public static IDataset[][] StitchingOverlapProcessMethod(ArrayList<IDataset> arrayILDy,
			ArrayList<IDataset> arrayILDx, OverlapUIModel model, boolean usenegativeQ) {
		...
		for (k = 0; k < arrayILDx.size() - 1; k++) {
			...
			double correctionRatio = PolynomialOverlap.correctionRatio(xLowerDataset, yLowerDataset, xHigherDataset,
					yHigherDataset, attenuationFactor);

			if (usenegativeQ) {

				correctionRatio = PolynomialOverlap.correctionRatio(xHigherDataset, yHigherDataset, xLowerDataset,
						yLowerDataset, attenuationFactor);
			...
			// System.out.println("attenuation factor: " + attenuationFactor + "
			// k: " +k);
			// }
		}
	...
	public static double[][] maxMinArrayGenerator(ArrayList<IDataset> arrayILDx, OverlapUIModel model) {
