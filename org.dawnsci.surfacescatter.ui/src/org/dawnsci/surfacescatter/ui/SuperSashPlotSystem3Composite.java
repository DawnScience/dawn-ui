...
//	private Button resetCrossHairs;
	...
	public void generalUpdate(int[][] lenPt) {
		...
		IRectangularROI greenRectangle = new RectangularROI(lenPt[1][0], lenPt[1][1], lenPt[0][0], lenPt[0][1], 0);
		...
		IDataset output = ssp.presenterDummyProcess(selection, ssp.getImage(selection), 3, lenPt);
		try {
			ILineTrace lt3 = VerticalHorizontalSlices.horizontalsliceBackgroundSubtracted(
					horizontalSlice.getROI().getBounds(), plotSystem1, ssp.getTemporaryBackground(), greenRectangle);

			ILineTrace lt4 = VerticalHorizontalSlices.verticalsliceBackgroundSubtracted(
					verticalSlice.getROI().getBounds(), getPlotSystem3(), ssp.getTemporaryBackground(), greenRectangle);

			ILineTrace lt5 = VerticalHorizontalSlices.horizontalsliceBackgroundSubtracted(
					horizontalSlice.getROI().getBounds(), plotSystem1, output, greenRectangle);
	...
	public void addOutPutCurvesWindow() {
		...
		try {
			outputCurves = new MultipleOutputCurvesTableView(topRight, SWT.FILL, 0, ssp);
