...
public class FastMaskTool extends AbstractToolPage {
	...
	private ThresholdMaskComposite thresholdMaskComposite;
	...
	public void createControl(Composite parent) {
		...
		thresholdMaskComposite = new ThresholdMaskComposite(tabFolder, SWT.None);
	...
	private static void includeThreshold(BooleanDataset mask, Double lower, Double upper, Dataset data) {
		if (!Arrays.equals(mask.getShape(), data.getShape())) throw new IllegalArgumentException("must have same shape");


		SingleInputBroadcastIterator iterator = new SingleInputBroadcastIterator(data, mask);
		iterator.setOutputDouble(true);

		final double l = lower == null ? Double.NEGATIVE_INFINITY : lower.doubleValue();
		final double h = upper == null ? Double.POSITIVE_INFINITY : upper.doubleValue();

		while (iterator.hasNext()) {

			double element = iterator.aDouble;

			if (element < l || element > h) {
				mask.setAbs(iterator.oIndex, false);
			}
		}
	}
	...
	private class ImageComposite extends Composite {
		...
		public ImageComposite(Composite parent, int style) {
			...
			Button b4 = new Button(this, SWT.PUSH);
			b4.setText("Dilate");
			b4.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					Runnable r = () -> {
						IImageFilterService service = Activator.getService(IImageFilterService.class);
						BooleanDataset mask = buffer.getMask();
						Double lower = lowerValue.get();
						Double upper = upperValue.get();
						...
						//Masked values are 0 so needs to be inverted for dilation
						Comparisons.logicalNot(mask,mask);
						mask = (BooleanDataset) service.filterDilate(mask, true);
						Comparisons.logicalNot(mask,mask);
						buffer.merge(mask);
					};
	...
	private class ThresholdMaskComposite extends Composite {
		...
		public ThresholdMaskComposite(Composite parent, int style) {
			...
			lower.addTraverseListener(new TraverseListener() {
				...
				public void keyTraversed(TraverseEvent e) {
					if (e.detail != SWT.TRAVERSE_RETURN) return;
					update(lowerValue,lower.getText());
			...
			upper.addTraverseListener(new TraverseListener() {
				...
				public void keyTraversed(TraverseEvent e) {
					if (e.detail != SWT.TRAVERSE_RETURN) return;
					update(upperValue,upper.getText());
		...
		private void update(AtomicReference<Double> value, String text) {
