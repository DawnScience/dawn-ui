...
public class NipyBlueTransferFunction extends AbstractTransferFunction {
		...
		for (int i = 0; i < HistogramData.NIPY_SPECTRAL.length; i++) {
			if (i > 0 && value <= HistogramData.NIPY_SPECTRAL[i][0]) {
				return HistogramData.interpolatedY(
						new double[] { HistogramData.NIPY_SPECTRAL[i - 1][0], HistogramData.NIPY_SPECTRAL[i - 1][3] },
						new double[] { HistogramData.NIPY_SPECTRAL[i][0], HistogramData.NIPY_SPECTRAL[i][3] }, value);
			}
		}
		return 0;
	}
