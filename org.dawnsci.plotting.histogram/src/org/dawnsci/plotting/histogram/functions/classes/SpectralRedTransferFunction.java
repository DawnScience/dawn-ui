...
public class SpectralRedTransferFunction extends AbstractTransferFunction {
		...
		for (int i = 0; i < HistogramData.SPECTRAL.length; i++) {
			if (i > 0 && value <= HistogramData.SPECTRAL[i][0]) {
				return HistogramData.interpolatedY(
						new double[] { HistogramData.SPECTRAL[i - 1][0], HistogramData.SPECTRAL[i - 1][1] },
						new double[] { HistogramData.SPECTRAL[i][0], HistogramData.SPECTRAL[i][1] }, value);
			}
		}
		return 0;
	}
