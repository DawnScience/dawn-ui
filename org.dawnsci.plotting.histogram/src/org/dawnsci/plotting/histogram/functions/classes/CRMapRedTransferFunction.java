...
public class CRMapRedTransferFunction extends AbstractTransferFunction {
		...
		for (int i = 0; i < HistogramData.CRMAP.length; i++) {
			if (i > 0 && value <= HistogramData.CRMAP[i][0]) {
				return HistogramData.interpolatedY(
						new double[] { HistogramData.CRMAP[i - 1][0], HistogramData.CRMAP[i - 1][1] },
						new double[] { HistogramData.CRMAP[i][0], HistogramData.CRMAP[i][1] }, value);
			}
		}
		return 0;
	}
