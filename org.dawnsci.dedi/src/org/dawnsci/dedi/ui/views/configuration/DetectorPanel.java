...
public class DetectorPanel implements Observer {
	...
	private static final List<Unit<Length>> PIXEL_SIZE_UNITS = Arrays.asList(UnitUtils.MILLIMETRE, UnitUtils.MICROMETRE);
		...
		detectorTypesComboViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection.size() > 0) {
					...
					pixelSizeValueLabel.setText(TextUtil.format(UnitUtils.convert(detector.getxPixelSize(), unit))
							+ " x " + TextUtil.format(UnitUtils.convert(detector.getyPixelSize(), unit)));
		...
		unitsComboViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection.size() > 0) {
					...
					DiffractionDetector detector = (DiffractionDetector) detectorTypesComboViewer
