...
public class ConfigurationView extends ViewPart implements Observer {
	...
	public void createPartControl(Composite parent) {
		...
		ComboUnitsProvider<Angle> combo = new ComboUnitsProvider<>(angleGroup, Arrays.asList(SI.RADIAN, NonSI.DEGREE_ANGLE));
		angle = new TextWithUnits<>(angleGroup, "Angle:", combo);
		angle.addAmountChangeListener(this::angleChanged);
		angle.setValue(UnitUtils.getQuantity(90, NonSI.DEGREE_ANGLE)); // Default value (including for all the beamline templates).
	...
	private void angleChanged() {
		Quantity<Angle> newAngle = angle.getValue(SI.RADIAN);
		if(newAngle == null) ResultsService.getInstance().getBeamlineConfiguration().setAngle(null);
		else ResultsService.getInstance().getBeamlineConfiguration().setAngle(newAngle.getValue().doubleValue());
	}
	...
	public void update(Observable o, Object arg) {
		...
		cameraLengthValueSpinner.setValues(
